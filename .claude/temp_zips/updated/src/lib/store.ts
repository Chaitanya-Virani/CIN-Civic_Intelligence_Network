...
const KEYS = {
  ...
  adminSession: "cin.adminSession.v1", // true | false — platform-admin auth
};
...
/* --------------------------- admin session -------------------------- */
// Separate from the per-tenant member session above: the admin console is a
// platform-level area (manages every tenant), so it isn't scoped to one
// tenant the way a member login is.
