...
export const auth = {
  ...
  /**
   * Client self-signup: creates a brand-new tenant-scoped member account.
   * Mirrors `seedNewTenant` below — writes through the same provisioned-users
   * seam, so the new account behaves exactly like a demo account afterwards
   * (SessionContext.login still does the actual session write).
   */
  signup(
    tenantId: string,
    input: { name: string; role: string; constituency: string },
  ): Promise<User> {
    const user: User = {
      id: `signup-${tenantId}-${Date.now().toString(36)}`,
      tenantId,
      name: input.name,
      role: input.role || "Member",
      constituency: input.constituency || "General",
      avatarInitials: initials(input.name),
    };
    addProvisionedUsers([user]);
...
export const tenants = {
  ...
  create(input: CreateTenantInput): Promise<Tenant> {
    const id = input.slug ? slugify(input.slug) : uniqueTenantId(input.name);
    const tenant: Tenant = {
      id,
      name: input.name,
